...
apt-get update
apt-get  install -y apache2 libapache2-mod-php  rrdtool mariadb-server snmp snmpd php php-mysql  libapache2-mod-php  php-intl php-snmp php-xml php-mbstring php-json php-gd php-gmp php-zip php-ldap 
 ...
 if [[  $plugins == "yes"  ]]
  then
   git clone https://github.com/Cacti/plugin_thold.git thold
    ...
    git clone https://github.com/Cacti/plugin_intropage.git intropage
    git clone https://github.com/Cacti/plugin_mikrotik.git mikrotik
   chown -R $user:$user thold
    chown -R $user:$user monitor
     chown -R $user:$user audit
     chown -R $user:$user webseer
     chown -R $user:$user flowview
     chown -R $user:$user intropage
     chown -R $user:$user mikrotik
     mv thold $location/cacti/plugins
      mv monitor $location/cacti/plugins
       mv audit $location/cacti/plugins
       mv webseer $location/cacti/plugins
       mv flowview $location/cacti/plugins
       mv intropage $location/cacti/plugins
       mv mikrotik $location/cacti/plugins
else
